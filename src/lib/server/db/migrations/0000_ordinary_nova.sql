...
CREATE TABLE `InsumosOcorrencias` (
	...
	FOREIGN KEY (`ferramenta_ref`) REFERENCES `Insumos`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `Usuarios` (
	...
	`criado_em` integer NOT NULL,
	`atualizado_em` integer NOT NULL,
	CONSTRAINT "cargo" CHECK("Usuarios"."cargo" in (0, 1))
);
--> statement-breakpoint
